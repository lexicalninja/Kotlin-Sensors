...
class CyclingSerializer {
    ...
    companion object {
        fun readSensorLocation(bytes: ByteArray): SensorLocation {
            return SensorLocation.values()[bytes.first().toInt()]
        }
        ...
        fun calculateWheelKPH(current: CyclingMeasurementData, previous: CyclingMeasurementData,
            ...
            var wheeltimeDelta: Short
            val wheelTimeSeconds: Double
            val wheelRPM: Double
            var cwr1: Int = if (current.cumulativeWheelRevolutions != null) {
                current.cumulativeWheelRevolutions!!
            } else {
                return null
            }
            var cwr2: Int = if (previous.cumulativeWheelRevolutions != null) {
                previous.cumulativeWheelRevolutions!!
            } else {
                return null
            }
            var lwet1: Short = if (current.lastWheelEventTime != null) {
                current.lastWheelEventTime!!
            } else {
                return null
            }
            var lwet2: Short = if (previous.lastWheelEventTime != null) {
