...
import com.kinetic.fit.ui.root.RootActivity
import java.util.*
...
//        Log.d(TAG, "Starting LEScan")
    ...
    private fun startActivityTimer() {
        ...
        activityUpdateTimer.scheduleAtFixedRate(ActivityTimerTask(), Date(), ActivityInterval.toLong())
    }
    ...
    private var scanCallback = object : ScanCallback() {
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            if (callbackType == ScanSettings.CALLBACK_TYPE_ALL_MATCHES) {
                val s = sensorForDevice(result.device, true)
            }
//            Log.d(TAG, "${sensors.map { it.device?.address }}")
        }
    }

    private fun sensorForDevice(device: BluetoothDevice, create: Boolean, advertisements: Array<UUID> = arrayOf()): BleSensor? {
        if (sensorsById[device.address] != null) return sensorsById[device.address]
        if (!create) return null
        val sensor = createEntity(SensorType.constructors.first(), device, applicationContext, arrayOf<UUID>())
    ...
    inner class ServiceFactory {
        ...
        var serviceUUIDs: MutableList<UUID>? = mutableListOf()
            get() {
                return if (serviceTypes.isEmpty()) null else serviceTypes.map { UUID.fromString(it.key) }.toMutableList()
            }
